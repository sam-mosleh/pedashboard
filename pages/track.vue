    ...
    <v-row class="py-6 px-3 justify-end align-center">
      <v-col cols="auto">
        <v-dialog transition="dialog-top-transition" max-width="600">
          ...
          <template v-slot:default="dialog">
            <v-card>
              <v-toolbar color="black" dark class="mb-8"
                >Your previous searches</v-toolbar
              >
              <v-card-text
                v-for="(searchedItem, index) in searchedItems"
                :key="index"
              >
                <div class="d-flex flex-row">
                  ...
                  <div class="text-12 align-center">
                    {{ searchedItem.value }}
                  </div>
                </div>
              </v-card-text>
      ...
      <v-dialog v-model="dialogAddCompany" max-width="500px">
        <v-card>
          ...
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn color="blue darken-1" text @click="dialogAddCompany = false">
              Cancel
            </v-btn>
            <v-btn color="blue darken-1" text @click="setStartForTracking">
              Save
            </v-btn>
          </v-card-actions>
        </v-card>
...
export default {
  ...
  methods: {
    ...
    setStartForTracking() {
      ...
      const companyIdValue = this.allCompanies.filter(
        (companyClone) => companyClone.name === this.newTrackCompany.name
      );
      ...
      this.newTrackCompany["kpi1"] = [700, 675, 710, 990, 910, 760, 750, 740];
      this.newTrackCompany["kpi2"] = [520, 655, 730, 670, 960, 580, 760, 750];
      this.newTrackCompany["kpi3"] = [660, 735, 720, 780, 980, 540, 590, 530];
      this.newTrackCompany["kpi4"] = [760, 685, 540, 780, 990, 720, 510, 770];
      if (localStorage.getItem("startedForTracking")) {
        startedForTracking = JSON.parse(
          localStorage.getItem("startedForTracking")
        );
        if (
          !startedForTracking.filter(
            (itemClone) => itemClone === this.newTrackCompany
          ).length
        ) {
